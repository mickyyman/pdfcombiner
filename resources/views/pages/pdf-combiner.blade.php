<x-filament-panels::page>
    ...
    {{-- These inline vars are parsed before @filamentScripts runs at       --}}
    {{-- end-of-body, so pdf-combiner.js always finds them defined.         --}}
    {{-- ------------------------------------------------------------------ --}}
    <script>
    ...
    {{-- ------------------------------------------------------------------ --}}
    <style>
        /* Scoped CSS variables & overrides for the PDF Combiner widget */
        .pdfcomb-wrap {
            --pc-accent:      #7c3aed;
            --pc-accent-h:    #6d28d9;
            --pc-danger:      #ef4444;
        ...
        .pdfcomb-drop-zone:hover,
        .pdfcomb-drop-zone.drag-over {
        ...
        .pdfcomb-step-connector { width: 48px; height: 2px; background: var(--pc-border); flex-shrink: 0; transition: background .3s; }
        ...
        /* Status bar */
        .pdfcomb-status { background: var(--pc-surface); border: 1px solid var(--pc-border); border-radius: 8px;
                          padding: .6rem 1rem; display: flex; align-items: center; gap: .75rem;
                          font-size: .85rem; color: var(--pc-muted); margin-top: .75rem; }
        .pdfcomb-status.hidden { display: none; }
        .pdfcomb-spinner { width: 1rem; height: 1rem; border-radius: 50%; flex-shrink: 0;
                           border: 2px solid var(--pc-border); border-top-color: var(--pc-accent);
                           animation: pdfcomb-spin .7s linear infinite; }
        @keyframes pdfcomb-spin { to { transform: rotate(360deg); } }

        /* Filter notice */
        .pdfcomb-filter-notice {
            background: rgba(245,158,11,.08); border: 1px solid rgba(245,158,11,.25);
            border-radius: 8px; padding: .55rem .85rem; font-size: .8rem; color: var(--pc-warning); margin-bottom: .75rem;
        }
        ...
        .pdfcomb-search-input {
            flex: 1; background: transparent; border: none; outline: none;
            padding: .45rem .5rem; font-size: .875rem; color: var(--pc-text);
        }
        ...
        /* Preview grid */
        #pdfcomb-previewPages {
            display: grid;
            grid-template-columns: repeat(2, 1fr);
            gap: .5rem;
        }
        @media (min-width: 640px)  { #pdfcomb-previewPages { grid-template-columns: repeat(3, 1fr); } }
        @media (min-width: 768px)  { #pdfcomb-previewPages { grid-template-columns: repeat(4, 1fr); } }

        .pdfcomb-preview-card {
            background: var(--pc-surface); border: 1px solid var(--pc-border);
            border-radius: 8px; padding: .4rem;
            transition: border-color .2s;
        }
        .pdfcomb-preview-card.page-highlight {
            border-color: var(--pc-warning) !important;
            box-shadow: 0 0 0 2px rgba(245,158,11,.35);
        }
        .pdfcomb-preview-num {
            font-size: .68rem; color: var(--pc-muted); margin-bottom: .25rem;
        ...
        .pdfcomb-match-badge {
            background: var(--pc-warning); color: #000; font-weight: 700;
            padding: .05rem .25rem; border-radius: 3px; font-size: .65rem; margin-left: .25rem;
        }

        /* Wizard nav divider */
        .pdfcomb-nav { display: flex; justify-content: space-between; align-items: center;
                       margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid var(--pc-border); }

        /* Modal overlay */
        .pdfcomb-modal-overlay {
        ...
        .pdfcomb-modal-header { display: flex; align-items: center; justify-content: space-between;
                                padding: 1rem 1.25rem; border-bottom: 1px solid var(--pc-border); }
        .pdfcomb-modal-title  { font-weight: 600; font-size: .95rem; color: var(--pc-text); }
        .pdfcomb-modal-body   { padding: 1.25rem; }
        .pdfcomb-modal-footer { display: flex; justify-content: flex-end; gap: .5rem;
                                padding: .75rem 1.25rem; border-top: 1px solid var(--pc-border); }
        .pdfcomb-form-label   { font-size: .78rem; color: var(--pc-muted); display: block; margin-bottom: .25rem; }
        .pdfcomb-form-ctrl {
            width: 100%; background: transparent; border: 1px solid var(--pc-border);
            border-radius: 8px; padding: .4rem .7rem; font-size: .875rem; color: var(--pc-text);
        ...
        .pdfcomb-toast {
            ...
            padding: .75rem 1.1rem; font-size: .875rem; max-width: 340px;
            box-shadow: 0 8px 24px rgba(0,0,0,.4);
        }
    </style>

    {{-- ------------------------------------------------------------------ --}}
    {{-- Main PDF Combiner UI                                                --}}
    {{-- ------------------------------------------------------------------ --}}
    <div
        class="pdfcomb-wrap"
        x-data="{
    ...
    >
        ...
        <div class="flex items-center justify-center gap-0 mb-6">
            <div class="pdfcomb-step active flex items-center gap-2" id="pdfcomb-si-1">
                <div class="pdfcomb-step-circle" id="pdfcomb-sc-num-1">1</div>
                <div class="pdfcomb-step-label">Add Files</div>
            </div>
            <div class="pdfcomb-step-connector mx-2" id="pdfcomb-sc-line-1"></div>
            <div class="pdfcomb-step flex items-center gap-2" id="pdfcomb-si-2">
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-6 shadow-sm">
                ...
                <div class="pdfcomb-drop-zone" id="pdfcomb-dropZone">
                    <div class="mb-3">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                             stroke="currentColor" class="mx-auto w-12 h-12" style="color:var(--pc-accent)">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                  d="M12 16.5V9.75m0 0 3 3m-3-3-3 3M6.75 19.5a4.5 4.5 0 0 1-1.41-8.775 5.25 5.25 0 0 1 10.338-2.32 5.75 5.75 0 0 1 1.33 11.094"/>
                        </svg>
                    </div>
                    <div class="font-semibold mb-1" style="color:var(--pc-text)">Drag &amp; Drop PDF files here</div>
                    <div class="text-sm mb-4" style="color:var(--pc-muted)">or pick a folder</div>
                    ...
                    <input type="file" id="pdfcomb-folderInput" accept=".pdf" multiple
                           style="display:none;" webkitdirectory directory>
                </div>

                <div id="pdfcomb-step1Summary" style="display:none;" class="mt-3">
                    <div class="flex items-center justify-between">
                        <span class="text-sm" style="color:var(--pc-muted)">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                                 class="inline w-4 h-4 mr-1" style="color:var(--pc-success)">
            ...
            <div class="pdfcomb-step-panel" id="pdfcomb-panel-2">
                ...
                <div class="flex items-center justify-between flex-wrap gap-2 mb-4">
                    <span class="font-semibold text-sm" style="color:var(--pc-text)">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                             class="inline w-4 h-4 mr-1" style="color:var(--pc-accent)">
                            <path d="M10 12.5a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5z"/>
                            <path fill-rule="evenodd" d="M.664 10.59a1.651 1.651 0 0 1 0-1.186A10.004 10.004 0 0 1 10 3c4.257 0 7.893 2.66 9.336 6.41.147.381.146.804 0 1.186A10.004 10.004 0 0 1 10 17c-4.257 0-7.893-2.66-9.336-6.41zM14 10a4 4 0 1 1-8 0 4 4 0 0 1 8 0z" clip-rule="evenodd"/>
                        </svg>
                        Preview
                        <span id="pdfcomb-previewPageCount" class="font-normal text-xs ml-1" style="color:var(--pc-muted)"></span>
                    </span>
                    <div class="flex items-center gap-2 flex-wrap">
                        <button class="pdfcomb-btn-ghost text-xs" id="pdfcomb-rerenderPreviewBtn" style="padding:.3rem .7rem;">
                        ...
                        <button class="pdfcomb-btn-ghost text-xs" id="pdfcomb-emailBtn" style="padding:.3rem .7rem;">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-3.5 h-3.5">
                        ...
                        <button class="pdfcomb-btn-accent text-sm" id="pdfcomb-downloadBtn" style="padding:.38rem .9rem;">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4">
                ...
                <div class="mb-3">
                    <div class="pdfcomb-search-wrap">
                        ...
                        <input type="text" class="pdfcomb-search-input" id="pdfcomb-searchInput"
                               placeholder="Search pages (comma-separated terms)…">
                    ...
                    <div id="pdfcomb-searchResults" style="display:none;"
                         class="mt-1 text-xs" style="color:var(--pc-muted);"></div>
                </div>
                ...
                <div class="pdfcomb-status hidden" id="pdfcomb-previewStatus">
                    ...
                    <span id="pdfcomb-previewStatusText">Generating preview…</span>
                </div>

                <div id="pdfcomb-previewPages" class="mt-2"></div>

                <div class="pdfcomb-status hidden" id="pdfcomb-dlStatus">
                    <div class="pdfcomb-spinner"></div>
                    <span id="pdfcomb-dlStatusText">Processing…</span>
                </div>
        ...
        {{-- Footer note --}}
        <p class="text-center text-xs mt-4" style="color:var(--pc-muted)">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
        ...
        >
            <div class="pdfcomb-modal-box">
                <div class="pdfcomb-modal-header">
                    ...
                    <button class="pdfcomb-btn-ghost" style="padding:.2rem .45rem;border-radius:6px;"
                            x-on:click="emailModalOpen = false">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4">
                            <path d="M6.28 5.22a.75.75 0 0 0-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 1 0 1.06 1.06L10 11.06l3.72 3.72a.75.75 0 1 0 1.06-1.06L11.06 10l3.72-3.72a.75.75 0 0 0-1.06-1.06L10 8.94 6.28 5.22z"/>
                        </svg>
                    </button>
                ...
                <div class="pdfcomb-modal-body">
                    <div class="mb-3">
                        ...
                        <input type="email" class="pdfcomb-form-ctrl" id="pdfcomb-emailTo"
                               placeholder="recipient@example.com">
                    </div>
                    <div class="mb-3">
                        <label class="pdfcomb-form-label" for="pdfcomb-emailSubject">Subject</label>
                        <input type="text" class="pdfcomb-form-ctrl" id="pdfcomb-emailSubject"
                               value="Merged PDF">
                    </div>
                    <div class="mb-3">
                        <label class="pdfcomb-form-label" for="pdfcomb-emailBody">Message</label>
                        <textarea class="pdfcomb-form-ctrl" id="pdfcomb-emailBody" rows="3"
                                  style="resize:vertical;">Please find the merged PDF attached.</textarea>
                    </div>
                    <div class="rounded-lg p-3 text-xs" style="background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.25);color:var(--pc-warning);">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="inline w-3.5 h-3.5 mr-1">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 1 1-16 0 8 8 0 0 1 16 0zm-7-4a1 1 0 1 1-2 0 1 1 0 0 1 2 0zM9 9a.75.75 0 0 0 0 1.5h.253a.25.25 0 0 1 .244.304l-.459 2.066A1.75 1.75 0 0 0 10.747 15H11a.75.75 0 0 0 0-1.5h-.253a.25.25 0 0 1-.244-.304l.459-2.066A1.75 1.75 0 0 0 9.253 9H9z" clip-rule="evenodd"/>
                        </svg>
                        Browsers cannot attach files to email. The PDF will be <strong>downloaded first</strong> — attach it manually after your mail client opens.
                    </div>
                </div>
                <div class="pdfcomb-modal-footer">
                    <button class="pdfcomb-btn-ghost text-sm" x-on:click="emailModalOpen = false">Cancel</button>
                    <button class="pdfcomb-btn-accent text-sm" id="pdfcomb-sendEmailBtn"
                            x-on:click="window.pdfCombinerSendEmail()">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4">
